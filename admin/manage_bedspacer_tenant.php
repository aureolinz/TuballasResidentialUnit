...
<div class="container-fluid">
	<form action="" id="manage-tenant">
		...
		<div class="form-group row">
			<div class="col-md-4">
				<label for="" class="control-label">Government ID <?php isset($id) ? '(Leave Blank if not changing.)' : '' ?></label>
		...
		<div class="form-group row">
			<div class="col-md-4">
				<input type="hidden" name="type"  value="Bedspacer">
				<label for="" class="control-label">Bedspacer</label>
				<select name="house_id" id="" class="custom-select select2"  onchange="getDownpayment(this.value)">
					<option value=""></option>
					<?php 
					$house = $conn->query("SELECT * FROM bedspacer where bedspacer_id not in (SELECT house_id from tenants where status = 1 AND type = 'Bedspacer') ".(isset($house_id)? " or bedspacer_id = $house_id": "" )." ");
					while($row= $house->fetch_assoc()):
					?>
					<option value="<?php echo $row['bedspacer_id'] ?>" <?php echo isset($house_id) && $house_id == $row['bedspacer_id'] ? 'selected' : '' ?>><?php echo $row['bedspacer_no'] ?></option>
					<?php endwhile; ?>
				</select>
			</div>
			<div class="col-md-4">
...
<script>
	...
	httpRequest.open("POST", "getBedspacerDownpayment.php");
