...
function getXML() {
    ...
    httpRequest.open("POST", "../chatMessagesXML/"+sessionStorage.getItem("email"));
    httpRequest.send();
}
...
function displayChat(xml) {
    ...
    for(let counter=0; counter<messages.length; counter++){
        ...
        if(messages[counter].getAttribute("sender") == "Admin"){
            messageTag.classList.add("your-message");
            messageText = messages[counter].innerHTML;
        } else {
            messageTag.classList.add("other-user-message");
            messageText = sessionStorage.getItem(messages[counter].getAttribute("sender"))+ " : " +messages[counter].innerHTML;
        }
